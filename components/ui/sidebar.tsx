...
const SIDEBAR_WIDTH_STORAGE_KEY = "vs_sidebar_width";
  ...
  return (
    <button
      ...
      onPointerMove={(event) => {
        ...
        const distance = (event.clientX - resize.startX) * resize.direction;
        if (Math.abs(distance) >= 3) didDragRef.current = true;
        resize.currentWidth = Math.min(
          SIDEBAR_WIDTH_MAX,
          Math.max(SIDEBAR_WIDTH_MIN, resize.startWidth + distance),
        );
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 touch-none -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-sidebar-border sm:flex",
