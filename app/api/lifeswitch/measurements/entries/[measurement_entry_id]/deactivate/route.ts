...
export async function POST(
  req: NextRequest,
  ctx: { params: Promise<{ measurement_entry_id: string }> }
) {
  ...
  const { measurement_entry_id } = await ctx.params;

  const inUrl = new URL(req.url);
  const upstream = new URL(
    `${BRAINS_URL}/lifeswitch/measurements/entries/${encodeURIComponent(measurement_entry_id)}/deactivate`
  );
