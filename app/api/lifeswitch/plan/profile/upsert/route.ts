...
export async function POST(req: NextRequest) {
  ...
  const upstream = new URL(`${BRAINS_URL}/lifeswitch/plan/profile/upsert`);
  ...
  try {
    const r = await fetch(upstream.toString(), {
      method: "POST",
      headers: lifeSwitchUpstreamHeaders(rid, owner_user_id, { "content-type": req.headers.get("content-type") || "application/json; charset=utf-8" }),
      body: bodyText || "{}",
      cache: "no-store",
    });
