...
export async function POST(
  req: NextRequest,
  ctx: { params: Promise<{ workout_template_id: string; workout_template_exercise_id: string }> }
) {
  ...
  const { workout_template_id, workout_template_exercise_id } = await ctx.params;

  const upstream = new URL(
    `${BRAINS_URL}/lifeswitch/training/workout_templates/${encodeURIComponent(
      workout_template_id
