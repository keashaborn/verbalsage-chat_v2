...
export async function POST(req: NextRequest, ctx: { params: Promise<{ workout_template_exercise_id: string }> }) {
  const rid = req.headers.get("x-request-id") || randomUUID();

  const owner_user_id = await getLifeSwitchOwnerUserId(req);
  if (!owner_user_id) return unauthorizedLifeSwitch(rid);

  const { workout_template_exercise_id } = await ctx.params;
  const upstream = new URL(
    `${BRAINS_URL}/lifeswitch/training/workout_template_exercises/${encodeURIComponent(workout_template_exercise_id)}/segments/upsert`
  );
  ...
  for (const key of ["segment_index", "label", "default_weight", "default_reps"]) {
