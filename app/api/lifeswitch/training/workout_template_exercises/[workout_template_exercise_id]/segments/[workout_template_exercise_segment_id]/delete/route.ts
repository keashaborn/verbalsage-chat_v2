...
export async function POST(
  req: NextRequest,
  ctx: { params: Promise<{ workout_template_exercise_id: string; workout_template_exercise_segment_id: string }> }
) {
  ...
  const { workout_template_exercise_id, workout_template_exercise_segment_id } = await ctx.params;
  const upstream = new URL(
    `${BRAINS_URL}/lifeswitch/training/workout_template_exercises/${encodeURIComponent(
      workout_template_exercise_id
    )}/segments/${encodeURIComponent(workout_template_exercise_segment_id)}/delete`
  );
