...
export async function POST(
  req: NextRequest,
  ctx: { params: Promise<{ my_conditioning_prescription_id: string }> }
) {
  ...
  const { my_conditioning_prescription_id } = await ctx.params;

  const inUrl = new URL(req.url);
  const upstream = new URL(
    `${BRAINS_URL}/lifeswitch/training/my_conditioning_prescriptions/${encodeURIComponent(
  ...
  try {
    ...
    const bodyOut = await r.text().catch(() => "");
