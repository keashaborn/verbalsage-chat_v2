...
export async function POST(
  req: NextRequest,
  ctx: { params: Promise<{ training_session_id: string; training_set_log_id: string; training_set_log_segment_id: string }> }
) {
  ...
  const { training_session_id, training_set_log_id, training_set_log_segment_id } = await ctx.params;
  const upstream = new URL(
    `${BRAINS_URL}/lifeswitch/training/sessions/${encodeURIComponent(training_session_id)}/sets/${encodeURIComponent(
      training_set_log_id
    )}/segments/${encodeURIComponent(training_set_log_segment_id)}/delete`
  );
