...
export async function GET(req: NextRequest, ctx: { params: Promise<{ training_session_id: string }> }) {
  ...
  const upstream = new URL(`${BRAINS_URL}/lifeswitch/training/sessions/${encodeURIComponent(training_session_id)}/sets`);
