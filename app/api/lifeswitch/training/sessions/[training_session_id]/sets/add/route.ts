...
export async function POST(req: NextRequest, ctx: { params: Promise<{ training_session_id: string }> }) {
  ...
  const upstream = new URL(`${BRAINS_URL}/lifeswitch/training/sessions/${encodeURIComponent(training_session_id)}/sets/add`);
  ...
  for (const key of ["exercise_id", "exercise_name", "workout_template_id", "exercise_sort_order", "set_index", "set_type", "weight", "reps", "flags", "notes"]) {
