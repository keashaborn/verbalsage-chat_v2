...
export async function POST(
  req: NextRequest,
  ctx: { params: Promise<{ conditioning_session_log_id: string }> }
) {
  ...
  const { conditioning_session_log_id } = await ctx.params;

  const inUrl = new URL(req.url);
  const upstream = new URL(
    `${BRAINS_URL}/lifeswitch/training/conditioning_sessions/${encodeURIComponent(
