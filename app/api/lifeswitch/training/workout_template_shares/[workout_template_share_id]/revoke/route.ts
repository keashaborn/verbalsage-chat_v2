...
export async function POST(
  req: NextRequest,
  ctx: { params: Promise<{ workout_template_share_id: string }> }
) {
  const rid = req.headers.get("x-request-id") || randomUUID();
  const owner_user_id = await getLifeSwitchOwnerUserId(req);
  if (!owner_user_id) return unauthorizedLifeSwitch(rid);

  const { workout_template_share_id } = await ctx.params;
  const inUrl = new URL(req.url);
  const upstream = new URL(
    `${BRAINS_URL}/lifeswitch/training/workout_template_shares/${encodeURIComponent(workout_template_share_id)}/revoke`
  );
