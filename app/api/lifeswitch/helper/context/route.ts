...
export async function GET(req: NextRequest) {
  ...
  const route = String(inUrl.searchParams.get("route") || "/lifeswitch").trim();
  const days = clampInt(inUrl.searchParams.get("days"), 7, 90, 14);
  const today = String(inUrl.searchParams.get("today") || ymd(new Date()));
  const startDay = String(inUrl.searchParams.get("start_day") || daysAgo(days - 1));
