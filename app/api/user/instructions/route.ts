...
function normalize(raw: any): Preferences {
  return {
    ...
    response_length: enumValue(raw?.response_length, ["concise", "balanced", "detailed"] as const, "balanced"),
    technical_depth: enumValue(raw?.technical_depth, ["plain", "balanced", "expert"] as const, "balanced"),
    format: enumValue(raw?.format, ["auto", "prose", "bullets", "steps"] as const, "auto"),
    encouragement: enumValue(raw?.encouragement, ["minimal", "neutral"] as const, "neutral"),
  };
}
...
export async function GET(req: Request) {
  ...
  if (!owner) return NextResponse.json({ error: "unauthorized" }, { status: 401, headers: { "x-request-id": requestId } });
  const brains = process.env.BRAINS_URL || "http://172.31.32.171:8088";
  const upstream = await fetch(
    `${brains}/cards/${encodeURIComponent(owner)}?vantage_id=${SCOPE}&kinds=user_instructions&limit=1`,
    { method: "GET", headers: brainsUpstreamHeaders(requestId, owner), cache: "no-store" },
  );
  const raw = await upstream.text().catch(() => "");
  if (!upstream.ok) return NextResponse.json({ error: "preferences_unavailable" }, { status: 502, headers: { "x-request-id": requestId } });
  ...
  return NextResponse.json({ ok: true, ...preferences, updated_at }, { headers: { "x-request-id": requestId } });
}
...
export async function POST(req: Request) {
  ...
  const brains = process.env.BRAINS_URL || "http://172.31.32.171:8088";
  const upstream = await fetch(`${brains}/cards/${encodeURIComponent(owner)}?vantage_id=${SCOPE}`, {
    method: "POST",
    headers: brainsUpstreamHeaders(requestId, owner, { "Content-Type": "application/json" }),
    body: JSON.stringify({
  ...
  if (!upstream.ok) return NextResponse.json({ error: "preferences_save_failed" }, { status: 502, headers: { "x-request-id": requestId } });
  return NextResponse.json({ ok: true, ...preferences }, { headers: { "x-request-id": requestId } });
}
