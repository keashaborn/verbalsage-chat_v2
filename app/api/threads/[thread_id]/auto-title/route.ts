export const runtime = "nodejs";
export const dynamic = "force-dynamic";

import type { NextRequest } from "next/server";
import { NextResponse } from "next/server";
import {
  forbiddenThread,
  getRequestId,
  getThreadUserId,
  threadUpstreamHeaders,
  threadBelongsToUser,
  unauthorized,
  UUID_RE,
} from "@/app/api/threads/_threadAuth";

export async function POST(req: NextRequest, context: { params: Promise<{ thread_id: string }> }) {
  const requestId = getRequestId(req);

  const user_id = await getThreadUserId(req);
  if (!user_id) return unauthorized(requestId);

  const { thread_id } = await context.params;
  const tid = String(thread_id || "").trim();
  if (!UUID_RE.test(tid)) {
    return NextResponse.json(
      { error: "invalid thread_id" },
      { status: 400, headers: { "x-request-id": requestId } }
    );
  }

  const ownsThread = await threadBelongsToUser(req, tid, user_id, requestId);
  if (!ownsThread) return forbiddenThread(requestId);

  const BRAINS = process.env.BRAINS_URL || "http://172.31.32.171:8088";

  const r = await fetch(`${BRAINS}/threads/${encodeURIComponent(tid)}/auto-title`, {
    method: "POST",
    headers: threadUpstreamHeaders(req, requestId, user_id, { "Content-Type": "application/json" }),
    body: "{}",
    cache: "no-store",
  });

  const result = await r.json().catch(() => ({}));

  if (!r.ok) {
    return NextResponse.json(
      { error: `brains HTTP ${r.status}` },
      { status: 502 }
    );
  }


  return NextResponse.json({
    ok: true,
    thread_id: tid,
    title: String(result?.title || "New chat"),
    title_source: String(result?.title_source || "placeholder"),
    updated: result?.updated !== false,
    skipped: result?.skipped || null,
  });
}
